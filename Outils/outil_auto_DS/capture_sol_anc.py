...
"""
capture_sol.py
Flo 🌿 — adaptation pour Sol (12/05/2026)

Capture la conversation DeepSeek (Sol_anc) en cours dans Brave déjà ouvert,
et sauvegarde en .md dans Sol_anc/ du repo.
Sauvegardes partielles tous les 10 messages pour éviter toute perte.
...
Écrit le chemin du fichier produit dans last_capture_sol.txt
...
def nom_fichier(session_name: str, part: int = 0) -> Path:
    date     = datetime.now().strftime("%Y%m%d")
    nom_base = session_name.replace(" ", "_")
    existants = list(SOL_ANC_DIR.glob(f"sol_anc_{date}_*.md"))
    indice   = len(existants) + 1
    suffix = f"_part{part:03d}" if part > 0 else ""
    return SOL_ANC_DIR / f"sol_anc_{date}_{indice:02d}_{nom_base}{suffix}.md"
...
() => {
    ...
    document.querySelectorAll('div.ds-markdown-paragraph').forEach(el => {
...
def _sauver_partiel(tous_items: list, session_name: str, part: int):
    ...
    try:
        ...
        chemin = nom_fichier(session_name, part)
...
def extraire_messages(page, session_name: str = "session") -> list[dict]:
    ...
    try:
        ...
        while True:
            ...
            # Sauvegarde partielle tous les ~10 nouveaux messages
